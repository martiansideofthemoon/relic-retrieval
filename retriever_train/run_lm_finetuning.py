"""Fine-tuning RoBERTa encoders for retrieval tasks."""
...
def save_model(roberta_model, output_dir, args, global_step, tokenizer=None):
    # Take care of distributed/parallel training
    model_to_save = roberta_model
    model_to_save = model_to_save.module if hasattr(model_to_save, 'module') else model_to_save

    logger.info("Saving model checkpoint to %s", output_dir)
    torch.save(args, os.path.join(output_dir, 'training_args.bin'))

    prefix_dir = os.path.join(output_dir, "prefix_encoder")
    suffix_dir = os.path.join(output_dir, "suffix_encoder")
    os.makedirs(prefix_dir, exist_ok=True)
    os.makedirs(suffix_dir, exist_ok=True)
    model_to_save.prefix_encoder.save_pretrained(prefix_dir)
    model_to_save.suffix_encoder.save_pretrained(suffix_dir)
...
def train(args, parent_model, train_dataset, tokenizer):
    """ Train the model """
    if args.local_rank in [-1, 0]:
        try:
            tb_writer = SummaryWriter(logdir="retriever_train/runs/summary_%s" % args.job_id)
        except:
            tb_writer = SummaryWriter(log_dir="retriever_train/runs/summary_%s" % args.job_id)
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    save_model(model, output_dir, args, global_step, tokenizer=tokenizer)
...
def evaluate(args, parent_model, tokenizer, prefix=""):
    ...
    # multi-gpu evaluate
    if args.n_gpu > 1:
        parent_model = torch.nn.DataParallel(parent_model)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        curr_result = parent_model.evaluate(batch)
        for k, v in curr_result.items():
            results[k] += v
        nb_eval_steps += 1

    for k, v in results.items():
        results[k] = results[k] / nb_eval_steps
...
def main():
    ...
    prefix_model = model_class.from_pretrained(args.model_name_or_path,
                                               from_tf=bool('.ckpt' in args.model_name_or_path),
                                               config=config,
                                               cache_dir=args.cache_dir if args.cache_dir else None)
    suffix_model = model_class.from_pretrained(args.model_name_or_path,
                                               from_tf=bool('.ckpt' in args.model_name_or_path),
                                               config=config,
                                               cache_dir=args.cache_dir if args.cache_dir else None)

    prefix_model.to(args.device)
    suffix_model.to(args.device)

    parent_model = PrefixSuffixModel(args=args, prefix_encoder=prefix_model, suffix_encoder=suffix_model)
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, parent_model, train_dataset, tokenizer)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if not os.path.exists(output_dir) and args.local_rank in [-1, 0]:
            os.makedirs(output_dir)
        save_model(parent_model, output_dir, args, global_step, tokenizer)

        parent_model, tokenizer = init_parent_model(checkpoint_dir=args.output_dir,
                                                    args=args,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        while not eval_done:
            ...
            if not args.evaluate_specific:
                checkpoints = list(os.path.dirname(c) for c in sorted(glob.glob(args.output_dir + '/checkpoint-*/suffix_encoder', recursive=True)))
            ...
            for checkpoint in checkpoints:
                ...
                parent_model, _ = init_parent_model(checkpoint_dir=checkpoint,
                                                    args=args,
                                                    model_class=model_class)
                result = evaluate(args, parent_model, tokenizer, prefix=prefix)
                all_results[checkpoint] = (result["loss"], result["accuracy"], result["mean_rank"],
                                           result["R@1"], result["R@3"], result["R@5"], result["R@10"])

            sorted_results = [(k, v) for k, v in all_results.items()]
            sorted_results.sort(key=lambda x: x[1][0].item())

            # Also save some extra checkpoints based on "recall@1" performance
            top_rec_1_ckpts = sorted(sorted_results, key=lambda x: x[1][3], reverse=True)
            top_rec_1_ckpts = [x[0] for x in top_rec_1_ckpts[:args.save_total_limit]]
            ...
            if not args.evaluate_specific and args.do_delete_old and len(sorted_results) > args.save_total_limit:
                ...
                for res in sorted_results[args.save_total_limit:]:
                    if res[0] in top_rec_1_ckpts:
                        logger.info(f"Skipping {res[0]} since it's in top recall@1 ckpts...")
                        continue
                    elif os.path.exists(res[0]):
            ...
            if not args.evaluate_specific and len(sorted_results) > 0 and sorted_results[0][0] != top_checkpoint:
                command = "cp -r {}/* {}".format(sorted_results[0][0], args.output_dir)
