...
if not args.rewrite_cache and os.path.exists(f"{args.model}/{args.split}_with_ranks.json"):
    load_existing = True
...
for book_title, book_data in data.items():
    ...
    all_masked_quotes = build_lit_instance([x["quote"] for x in all_quotes], left_sents, right_sents, append_quote=False)
